...
import use.com.nettySupport.initialHandle.YSBayonetChildHandleInitial;

/**
 * 宇视服务器端
 * 项目名称:use.nettySupport
 * 类型名称:YSBayonetServer
 * 类型描述:
 * 作者:wx
 * 创建时间:2018年3月27日
 * @version:
...
public class YSBayonetServer implements Runnable , ISocketServerBean{

	public static String title = "宇视相机卡口SOCKET-";
	
	protected final Logger log = LoggerFactory.getLogger(QJSignalServer.class);
	
	private int policy_port = 5196;  
	 ...
	 */
	public YSBayonetServer(int port , Class businessClass , Class readIdleClass)
	{
	...
	public void run() {
		...
		try
		{
			...
			bootStrap.childHandler(new YSBayonetChildHandleInitial(fBusinessClass , fReadIdleClass));
	...
	public static ISocketServerBean openListener(Class businessClass ,Class readIdleClass) {
		YSBayonetServer r = new YSBayonetServer(SocketUtil.getYSBayontParameter().getPort(), businessClass , readIdleClass);
		if(SocketUtil.getYSBayontParameter().isEnable())
		{
			new Thread(r).start();
		}
		return r;
